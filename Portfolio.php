<?php include( "head.php"); ?>
<?php include( "menu.php"); ?>
<?php include( "inquire_form.php"); ?>
<div class="pagepiling">
	<div class="pp-scrollable section work_wrapper">
		<?php include( "header.php");?>
		<div class="scroll-wrap">
			<div class="section-bg" style="background-image: linear-gradient(rgba(0, 0, 0, 0.70), rgba(0, 0, 0, 0.70)), url(assets/images/bg/workbg.jpeg);background-attachment: fixed;"></div>
			<div class="scrollable-content position_contact">
				<div class="vertical-centred">
					<div class="boxed boxed-inner">
						<div class="vertical-title text-dark hidden-xs hidden-sm"><span>What we have done</span>
						</div>
						<div class="boxed">
							<div class="container">
								<div class="intro overflow-hidden">
									<div class="row">
										<div class="ourwork_head">
											<p>We are proud to offer both customers and our employees with the most up-to-date and cutting-edge technologies and partnerships.</p>
										</div>
										<div class="clearfix"></div>
										<!-- work Section -->
										<div class="work_wrapper_content">
											<div class="col-md-12">
												<h2 class="title-uppercase text-white visible-xs visible-sm">What we have done</h2>
												<div class="row-project-box row">
													<div class="afp col-project-box col-md-3 col-lg-3 col-sm-6 col-xs-6">
														<a href="PortfolioDetail?id=0" class="project-box">
															<div class="project-box-inner">
																<img alt="" class="img-responsive" src="assets/images/partners/afp.png">
																<!--<i class="fa fa-wrench" aria-hidden="true"></i>-->
																<h5>Corporate Training Skills Assessment</h5>
																<div class="project-category">Assessment Tool</div>
																<div class="work_detail"><i class="fa fa-arrow-right" aria-hidden="true"></i>
																</div>
																<!-- <p>AFP established and administers the Certified Treasury Professional and Certified Corporates.</p>-->
															</div>
														</a>
													</div>
													<div class="atd col-project-box col-md-3 col-lg-3 col-sm-6 col-xs-6">
														<a href="PortfolioDetail?id=1" class="project-box">
															<div class="project-box-inner">
																<img alt="" class="img-responsive" src="assets/images/partners/atd.png">
																<!--<i class="fa fa-wrench" aria-hidden="true"></i>-->
																<h5>Assessment Tool</h5>
																<div class="project-category">Scoring Tool for Assessing Team</div>
																<div class="work_detail"><i class="fa fa-arrow-right" aria-hidden="true"></i>
																</div>
																<!--<p>ATD has several awards programs and recognizes organizations and individuals who are leading the talent development profession.</p>-->
															</div>
														</a>
													</div>
													<div class="lfb col-project-box col-md-3 col-lg-3 col-sm-6 col-xs-6">
														<a href="PortfolioDetail?id=2" class="project-box">
															<div class="project-box-inner">
																<img alt="" class="img-responsive" src="assets/images/partners/lf.png">
																<!--<i class="fa fa-book" aria-hidden="true"></i>-->
																<h5>Learn Feedback</h5>
																<div class="project-category">Straehle Feedback Inventory</div>
																<div class="work_detail"><i class="fa fa-arrow-right" aria-hidden="true"></i>
																</div>
																<!--<p>Learnfeedback is a program built to assess, train, monitor, and coach individuals in your organization.</p>-->
															</div>
														</a>
													</div>
													<div class="alli col-project-box col-md-3 col-lg-3 col-sm-6 col-xs-6">
														<a href="PortfolioDetail?id=3" class="project-box">
															<div class="project-box-inner">
																<img alt="" class="img-responsive" src="assets/images/partners/all.png">
																<!--<i class="fa fa-user" aria-hidden="true"></i>-->
																<h5>All Institute</h5>
																<div class="project-category">Coaching Services</div>
																<div class="work_detail"><i class="fa fa-arrow-right" aria-hidden="true"></i>
																</div>
																<!--<p>ALL-Institute develops a curriculum and learning experience that is applied, real-world, problem-based, and represents the needs of the employers.</p>-->
															</div>
														</a>
													</div>
													<div class="ted col-project-box col-md-3 col-lg-3 col-sm-6 col-xs-6">
														<a href="PortfolioDetail?id=4" class="project-box">
															<div class="project-box-inner">
																<img alt="" class="img-responsive" src="assets/images/partners/ted.png">
																<!--<i class="fa fa-user" aria-hidden="true"></i>-->
																<h5>TEDxTysons</h5>
																<div class="project-category">Coaching Services</div>
																<div class="work_detail"><i class="fa fa-arrow-right" aria-hidden="true"></i>
																</div>
																<!--<p>TEDx is a program of local,self-organized events that bring people together to share a TED-like experience. </p>-->
															</div>
														</a>
													</div>
													<div class="aere col-project-box col-md-3 col-lg-3 col-sm-6 col-xs-6">
														<a href="PortfolioDetail?id=5" class="project-box">
															<div class="project-box-inner">
																<img alt="" class="img-responsive" src="assets/images/partners/aere.png">
																<!--<i class="fa fa-wrench" aria-hidden="true"></i>-->
																<h5>Assessment, Education &amp; Research Experts</h5>
																<div class="project-category">Event</div>
																<div class="work_detail"><i class="fa fa-arrow-right" aria-hidden="true"></i>
																</div>
																<!--<p>AERE provides assessment, education, and research consulting services to organizations.</p>-->
															</div>
														</a>
													</div>
													<div class="afp col-project-box col-md-3 col-lg-3 col-sm-6 col-xs-6">
														<a href="PortfolioDetail?id=6" class="project-box">
															<div class="project-box-inner">
																<img alt="" class="img-responsive" src="assets/images/partners/afp.png">
																<!--<i class="fa fa-plus"></i>-->
																<h5>Corporate Training Skills Assessment</h5>
																<div class="project-category">Assessment Tool</div>
																<div class="work_detail"><i class="fa fa-arrow-right" aria-hidden="true"></i>
																</div>
																<!--<p>AFP established and administers the Certified Treasury Professional and Certified Corporates.</p>-->
															</div>
														</a>
													</div>
													<div class="atd col-project-box col-md-3 col-lg-3 col-sm-6 col-xs-6">
														<a href="PortfolioDetail?id=7" class="project-box">
															<div class="project-box-inner">
																<img alt="" class="img-responsive" src="assets/images/partners/atd.png">
																<!--<i class="fa fa-wrench" aria-hidden="true"></i>-->
																<h5>Assessment Tool</h5>
																<div class="project-category">Scoring Tool for Assessing Team</div>
																<div class="work_detail"><i class="fa fa-arrow-right" aria-hidden="true"></i>
																</div>
																<!--<p>ATD has several awards programs and recognizes organizations and individuals who are leading the talent development profession.</p>-->
															</div>
														</a>
													</div>
													<!--<div class="atd col-project-box col-sm-6 col-md-3 col-lg-3">
													<a href="http://theopeneyes.com/atd" target="_blank" class="project-box">
														<div class="project-box-inner more_portfolio">
															<h5>Click Here Fore Complete Portfolio</h5>
															<div class="work_detail"><i class="fa fa-arrow-right" aria-hidden="true"></i></div>
															<p>ATD has several awards programs and recognizes organizations and individuals who are leading the talent development profession.</p>
														</div>
													</a>
												</div>--></div>
												<div class="clearfix"></div>
											</div>
										</div>
										<div class="clearfix"></div>
										<!-- End Map Section -->
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
<?php include( "footer.php"); ?>