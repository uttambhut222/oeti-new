<?php include("header.php");?>
<?php include("inquire_form.php"); ?>
<div class="pagepiling">
	<div class="pp-scrollable our_team">
		<div class="scroll-wrap">
		<div class="section-bg" style="background-image: linear-gradient(rgba(0, 0, 0, 0.80), rgba(0, 0, 0, 0.80)), url(assets/images/bg/teambg.jpg);background-attachment: fixed;"></div>
			<div class="scrollable-content">
				<div class="vertical-centred">
					<div class="boxed boxed-inner">
						<div class="vertical-title text-dark hidden-xs hidden-sm"><span>Meet our team</span>
						</div>
						<div class="boxed">
							<div class="container">
								<div class="intro">
									<div class="row">
										<div class="our_team_wrap">
											<div class="team_header">
												<h3>"The people who work at OpenEyes share the vision and values of our community."</h3>											
												<p>We’re driven by the idea that the best work is born from diligence, craftsmanship and fun. We brought together smart, talented thinkers and unleashed their combined creativity on the world.</p>
											</div>
											<div class="clearfix"></div>
											<div class="our_team_section">
												<div class="row">
												<div class="col-md-3">
														<div class="team_member_box">
															<img alt="" src="assets/images/team/12.jpg">
															<div class="employee_details">
																<h4 class="employee_name">Anand Yadav</h4>
																<span class="employee_designation">Front End Developer</span>
															</div>
														</div>
													</div>
													<div class="col-md-3">
														<div class="team_member_box">
															<img alt="" src="assets/images/team/12.jpg">
															<div class="employee_details">
																<h4 class="employee_name">Dharti Desai</h4>
																<span class="employee_designation">HR Executive</span>
															</div>
														</div>
													</div>
													<div class="col-md-3">
														<div class="team_member_box">
															<img alt="" src="assets/images/team/12.jpg">
															<div class="employee_details">
																<h4 class="employee_name">Jaxi Chawda</h4>
																<span class="employee_designation">Php Developer</span>
															</div>
														</div>
													</div>
													<div class="col-md-3">
														<div class="team_member_box">
															<img alt="" src="assets/images/team/11.jpg">
															<div class="employee_details">
																<h4 class="employee_name">Kamlesh Solanki</h4>
																<span class="employee_designation">Office Administrator</span>
															</div>
														</div>
													</div>
												</div>
												<div class="clearfix"></div>
												<div class="row">
													<div class="col-md-3">
														<div class="team_member_box">
															<img alt="" src="assets/images/team/12.jpg">
															<div class="employee_details">
																<h4 class="employee_name">Krupali Mistry</h4>
																<span class="employee_designation">Front End Developer</span>
															</div>
														</div>
													</div>
													<div class="col-md-3">
														<div class="team_member_box">
															<img alt="" src="assets/images/team/11.jpg">
															<div class="employee_details">
																<h4 class="employee_name">Mayank Sharma</h4>
																<span class="employee_designation">Technical Recruiter</span>
															</div>
														</div>
													</div>
													<div class="col-md-3">
														<div class="team_member_box">
															<img alt="" src="assets/images/team/11.jpg">
															<div class="employee_details">
																<h4 class="employee_name">Mitesh Patel</h4>
																<span class="employee_designation">Php Developer</span>
															</div>
														</div>
													</div>
													<div class="col-md-3">
														<div class="team_member_box">
															<img alt="" src="assets/images/team/12.jpg">
															<div class="employee_details">
																<h4 class="employee_name">Nirav Patel</h4>
																<span class="employee_designation">Php Developer</span>
															</div>
														</div>
													</div>													
												</div>
												<div class="clearfix"></div>
												<div class="row">													
													<div class="col-md-3">
														<div class="team_member_box">
															<img alt="" src="assets/images/team/11.jpg">
															<div class="employee_details">
																<h4 class="employee_name">Pooja Patel</h4>
																<span class="employee_designation">Project Lead</span>
															</div>
														</div>
													</div>
													<div class="col-md-3">
														<div class="team_member_box">
															<img alt="" src="assets/images/team/12.jpg">
															<div class="employee_details">
																<h4 class="employee_name">Swar Pandya</h4>
																<span class="employee_designation">HR Executive</span>
															</div>
														</div>
													</div>													
													<div class="col-md-3">
														<div class="team_member_box">
															<img alt="" src="assets/images/team/11.jpg">
															<div class="employee_details">
																<h4 class="employee_name">Uttam Bhut</h4>
																<span class="employee_designation">Php Developer</span>
															</div>
														</div>
													</div>
													<div class="col-md-3">
														<div class="team_member_box">
															<img alt="" src="assets/images/team/11.jpg">
															<div class="employee_details">
																<h4 class="employee_name">Vidhi Bathani</h4>
																<span class="employee_designation">Php Developer</span>
															</div>
														</div>
													</div>
												</div>
												<div class="clearfix"></div>
											</div>
										</div>
									</div>
									<div class="clearfix"></div>
								</div>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</div>
<?php include("footer.php"); ?>