<?php include("header.php");?>
<?php include("inquire_form.php"); ?>
<div class="pagepiling">
	<div id="myCarousel" class="carousel slide" data-ride="carousel">
		<!-- Wrapper for slides -->
		<div class="carousel-inner">
			<div class="item active">
				<img src="assets/images/bg/afpbg.jpg" alt="bg">
				<div class="carousel-caption">
					<h3>AFP CORPORATE TRAINING SKILLS ASSESSMENT</h3>
					<p>AFP established and administers the <strong>Certified Treasury Professional
					   </strong> and <strong>Certified Corporate FP&A Professional credentials,</strong>
					   which set standards of excellence in finance.</p>
					<div class="project_desc">
						<ul>
							<li>
								<span>Clients:</span>
								<span class="pull-right">AFP</span>
							</li>
							<li>
								<span>Completion:</span>
								<span class="pull-right">February 5th,2017</span>
							</li>
							<li>
								<span>Project Type:</span>
								<span class="pull-right">Corporate</span>
							</li>
						</ul>
					</div>
					<div class="technology_used">
						<div class="technology_title pull-left">
							<strong>Technology Used:</strong>
						</div>
						<div class="technology_link pull-right">
							<ul>
								<li title="" data-toggle="tooltip" data-placement="top" data-original-title="Angular"><img src="assets/images/services/angular.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="Codeigniter"><img src="assets/images/services/codeigniter.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="HTML"><img src="assets/images/services/html.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="Bootstrap"><img src="assets/images/services/bootstrap.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="CSS"><img src="assets/images/services/css.png" alt="" class="img-responsive"></li>
							</ul>
						</div>
					</div>
				</div>
			</div>
			<div class="item">
			  <img src="assets/images/bg/atdbg.jpg" alt="bg">
			  <div class="carousel-caption">
					<h3>ATD</h3>
					<p>ATD established and administers the <strong>Certified Treasury Professional
					   </strong> and <strong>Certified Corporate FP&A Professional credentials,</strong>
					   which set standards of excellence in finance.</p>
					<div class="project_desc">
						<ul>
							<li>
								<span>Clients:</span>
								<span class="pull-right">ATD</span>
							</li>
							<li>
								<span>Completion:</span>
								<span class="pull-right">Jun 12th,2015</span>
							</li>
							<li>
								<span>Project Type:</span>
								<span class="pull-right">Assessment</span>
							</li>
						</ul>
					</div>
					<div class="technology_used">
						<div class="technology_title pull-left">
							<strong>Technology Used:</strong>
						</div>
						<div class="technology_link pull-right">
							<ul>
								<li title="" data-toggle="tooltip" data-placement="top" data-original-title="Angular"><img src="assets/images/services/angular.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="Codeigniter"><img src="assets/images/services/codeigniter.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="HTML"><img src="assets/images/services/html.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="Bootstrap"><img src="assets/images/services/bootstrap.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="CSS"><img src="assets/images/services/css.png" alt="" class="img-responsive"></li>
							</ul>
						</div>
					</div>
				</div>
			</div>
			<div class="item">
			  <img src="assets/images/bg/lfbbg.jpg" alt="bg">
			  <div class="carousel-caption">
					<h3>LEARN FEEDBACK</h3>
					<p>Learnfeedback is a program built to assess, train, monitor, and coach individuals in your organization.</p>
					<div class="project_desc">
						<ul>
							<li>
								<span>Clients:</span>
								<span class="pull-right">LFB</span>
							</li>
							<li>
								<span>Completion:</span>
								<span class="pull-right">Jan 25th,2016</span>
							</li>
							<li>
								<span>Project Type:</span>
								<span class="pull-right">Learning</span>
							</li>
						</ul>
					</div>
					<div class="technology_used">
						<div class="technology_title pull-left">
							<strong>Technology Used:</strong>
						</div>
						<div class="technology_link pull-right">
							<ul>
								<li title="" data-toggle="tooltip" data-placement="top" data-original-title="Angular"><img src="assets/images/services/angular.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="Codeigniter"><img src="assets/images/services/codeigniter.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="HTML"><img src="assets/images/services/html.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="Bootstrap"><img src="assets/images/services/bootstrap.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="CSS"><img src="assets/images/services/css.png" alt="" class="img-responsive"></li>
							</ul>
						</div>
					</div>
				</div>
			</div>
			<div class="item">
			  <img src="assets/images/bg/allibg.jpg" alt="bg">
			  <div class="carousel-caption">
					<h3>APPLIED LIFELONG LEARNING INSTITUTE (ALL-INSTITUTE)</h3>
					<p>They provide a learning opportunity that is engaging, meaningful, and aligned to real-world work situations across the career lifespan.</p>
					<div class="project_desc">
						<ul>
							<li>
								<span>Clients:</span>
								<span class="pull-right">ALLI</span>
							</li>
							<li>
								<span>Completion:</span>
								<span class="pull-right">Aug 7th,2017</span>
							</li>
							<li>
								<span>Project Type:</span>
								<span class="pull-right">Institute</span>
							</li>
						</ul>
					</div>
					<div class="technology_used">
						<div class="technology_title pull-left">
							<strong>Technology Used:</strong>
						</div>
						<div class="technology_link pull-right">
							<ul>
								<li title="" data-toggle="tooltip" data-placement="top" data-original-title="Angular"><img src="assets/images/services/angular.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="Codeigniter"><img src="assets/images/services/codeigniter.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="HTML"><img src="assets/images/services/html.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="Bootstrap"><img src="assets/images/services/bootstrap.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="CSS"><img src="assets/images/services/css.png" alt="" class="img-responsive"></li>
							</ul>
						</div>
					</div>
				</div>
			</div>
			<div class="item">
			  <img src="assets/images/bg/tedxbg.jpg" alt="bg">
			  <div class="carousel-caption">
					<h3>TEDX TYSONS</h3>
					<p>TEDx is a program of local,self-organized events that bring people together to share a TED-like experience.</p>
					<div class="project_desc">
						<ul>
							<li>
								<span>Clients:</span>
								<span class="pull-right">TEDX</span>
							</li>
							<li>
								<span>Completion:</span>
								<span class="pull-right">Oct 12th,2014</span>
							</li>
							<li>
								<span>Project Type:</span>
								<span class="pull-right">Coaching</span>
							</li>
						</ul>
					</div>
					<div class="technology_used">
						<div class="technology_title pull-left">
							<strong>Technology Used:</strong>
						</div>
						<div class="technology_link pull-right">
							<ul>
								<li title="" data-toggle="tooltip" data-placement="top" data-original-title="Angular"><img src="assets/images/services/angular.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="Codeigniter"><img src="assets/images/services/codeigniter.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="HTML"><img src="assets/images/services/html.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="Bootstrap"><img src="assets/images/services/bootstrap.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="CSS"><img src="assets/images/services/css.png" alt="" class="img-responsive"></li>
							</ul>
						</div>
					</div>
				</div>
			</div>
			<div class="item">
			  <img src="assets/images/bg/aerebg.jpg" alt="bg">
			  <div class="carousel-caption">
					<h3>AERE - ASSESSMENT, EDUCATION & RESEARCH EXPERTS</h3>
					<p>AERE provides assessment, education, and research consulting services to organizations specializing in assessment, licensure, certification, employment selection, survey research, and data analysis.</p>
					<div class="project_desc">
						<ul>
							<li>
								<span>Clients:</span>
								<span class="pull-right">AERE</span>
							</li>
							<li>
								<span>Completion:</span>
								<span class="pull-right">Sept 16th,2017</span>
							</li>
							<li>
								<span>Project Type:</span>
								<span class="pull-right">Event</span>
							</li>
						</ul>
					</div>
					<div class="technology_used">
						<div class="technology_title pull-left">
							<strong>Technology Used:</strong>
						</div>
						<div class="technology_link pull-right">
							<ul>
								<li title="" data-toggle="tooltip" data-placement="top" data-original-title="Angular"><img src="assets/images/services/angular.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="Codeigniter"><img src="assets/images/services/codeigniter.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="HTML"><img src="assets/images/services/html.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="Bootstrap"><img src="assets/images/services/bootstrap.png" alt="" class="img-responsive"></li>
							    <li title="" data-toggle="tooltip" data-placement="top" data-original-title="CSS"><img src="assets/images/services/css.png" alt="" class="img-responsive"></li>
							</ul>
						</div>
					</div>
				</div>
			</div>
		</div>
		<!-- Left and right controls -->
		<a class="left carousel-control" href="#myCarousel" data-slide="prev">
			<span class="glyphicon glyphicon-chevron-left"></span>
			<span class="sr-only">Previous</span>
		</a>
		<a class="right carousel-control" href="#myCarousel" data-slide="next">
			<span class="glyphicon glyphicon-chevron-right"></span>
			<span class="sr-only">Next</span>
		</a>
	</div>
</div>
<?php include("footer.php"); ?>